...
export class AICompanionService {
  ...
  static async initialize() {
    try {
      ...
      if (aiInitialized) {
        ...
        console.log("⚠️ Intelligent AI Service not available, using fallback responses");
      }
      ...
      if (ELEVEN_LABS_CONFIG.isConfigured()) {
        ...
        console.log("⚠️ Eleven Labs API key not configured, TTS will be skipped");
      }
  ...
  static updateContext(newContext) {
    ...
    if (newContext.isNavigating && newContext.currentLocation && this.routeCheckpoints.length > 0) {
    ...
    console.log("🧠 AI Context updated:", this.context);
  ...
  static async speak(text, options = {}) {
    try {
      ...
      if (text && typeof text === 'object' && typeof text.then === 'function') {
        console.error("❌ Promise object passed to speak method. This indicates a missing 'await' keyword.");
      ...
      if (!text || typeof text !== 'string' || text.trim().length === 0) {
        console.log("⚠️ No valid text provided for TTS. Received:", typeof text, text);
        return;
      }
  ...
  static async stopListening() {
    try {
      ...
      // Give a moment for everything to stop, then speak goodbye
      setTimeout(async () => {
        await this.speak("I'll be here whenever you need me. Stay safe on your journey!", { priority: 'high' });
      }, 500);
    } catch (error) {
  ...
  static getFallbackResponse(userInput = "") {
    ...
    if (userInput.toLowerCase().includes("help") || userInput.toLowerCase().includes("scared")) {
    ...
    if (userInput.toLowerCase().includes("directions") || userInput.toLowerCase().includes("turn")) {
      if (nextTurn) {
    ...
    if (userInput.toLowerCase().includes("safe") || userInput.toLowerCase().includes("okay")) {
  ...
  static async provideNavigationUpdate() {
    ...
    if (nextTurn) {
      ...
      if (this.hasGivenNavigationInstructionForLocation(nextTurn.instruction)) {
        console.log("🔇 Skipping repeated navigation instruction for same location");
        return;
      }
      
      if (distance <= 200 && distance > 100) {
        await this.speak(`Coming up in ${nextTurn.distance}, ${nextTurn.instruction}. I'll let you know when we're closer!`);
      } else if (distance <= 100 && distance > 50) {
        await this.speak(`Get ready! In ${nextTurn.distance}, ${nextTurn.instruction}.`);
      } else if (distance <= 50) {
    ...
    if (routeProgress === 25) {
      ...
      await this.speak("Halfway point reached! You're making excellent progress. How are you feeling about the journey?");
    } else if (routeProgress === 75) {
      await this.speak("Three quarters complete! Almost there, stay strong! The end is in sight.");
    } else if (routeProgress >= 90) {
      await this.speak("You're almost at your destination! Just a little bit further. You've done amazing!");
    }
  }
  ...
  static async shareLocationInfo() {
    ...
    try {
      ...
      if (locationName) {
        ...
        await this.speak(`${timeGreeting} We're in ${locationName}. ${locationInfo}`);
      } else if (currentLocation) {
        await this.speak(`${timeGreeting} This looks like a nice area! I'm checking what interesting places are around here.`);
      }
  ...
  static async shareNearbyPlaces() {
    ...
    try {
      ...
      if (allPlaces.length === 0) {
        await this.speak("I'm still discovering what's around here. This area seems peaceful and quiet.");
        return;
      }
      ...
      if (restaurants.length > 0) {
        ...
        await this.speak(`If you're getting hungry, there's ${restaurant.name} nearby. It looks like a great spot for a meal!`);
      } else if (cafes.length > 0) {
        const cafe = cafes[0];
        await this.speak(`I spotted ${cafe.name} around here. Perfect if you need a coffee break or quick snack!`);
      }
      ...
      if (landmarks.length > 0) {
        setTimeout(async () => {
          if (this.isListening) {
            ...
            await this.speak(`Oh, and there's ${landmark.name} in this area. That's pretty cool! This neighborhood has some character.`);
          }
        }, 4000);
      }
  ...
  static async selectConversationTopic() {
    ...
    try {
      ...
      if (topic && typeof topic === 'string' && topic.trim().length > 0) {
        ...
        console.log("⚠️ Invalid topic returned from AI service, using fallback");
        return this.getFallbackConversationTopic();
      }
      
    } catch (error) {
      console.error("❌ Error generating intelligent conversation topic:", error);
  ...
  static getFallbackConversationTopic() {
    ...
    if (isNavigating) {
      if (routeProgress < 25) {
        topics.push("How are you feeling about the journey so far? The route looks good ahead.");
        topics.push("I'm keeping an eye on traffic and safety conditions for you. Everything's looking smooth!");
      } else if (routeProgress > 75) {
        topics.push("We're almost there! You've done great on this trip. How are you feeling?");
        topics.push("Just a little bit more to go! I hope you've enjoyed our conversation along the way.");
      } else {
        topics.push("We're making great progress! Is there anything you'd like to know about the area we're passing through?");
        topics.push("How's the drive going? I'm here if you want to chat about anything.");
      }
    }
    ...
    const hour = new Date().getHours();
    if (hour >= 6 && hour < 12) {
      topics.push("Beautiful morning for a journey! I love how the morning light makes everything look fresh.");
      topics.push("Hope you had a good breakfast! Morning travels are some of my favorites.");
    } else if (hour >= 12 && hour < 17) {
      topics.push("Perfect afternoon for being out and about! The weather seems nice for traveling.");
      topics.push("Afternoon adventures are the best! How has your day been so far?");
    } else if (hour >= 17 && hour < 22) {
      topics.push("Evening travels have their own charm, don't they? The day is winding down nicely.");
      topics.push("I love the golden hour lighting at this time of day. Perfect for a peaceful journey.");
    } else {
      topics.push("It's pretty quiet out here at night. I'm keeping extra watch to make sure you're safe.");
      topics.push("Night driving can be peaceful. I'm here to keep you company and alert.");
    }

    // Safety-related positive topics
    if (nearbySpots.some(spot => spot.type === 'police')) {
      topics.push("I noticed there are police stations in this area. Always good to know we're in a well-monitored neighborhood!");
    }

    if (nearbySpots.some(spot => spot.type === 'hospital')) {
      topics.push("This area has good medical facilities nearby. It's always reassuring to be in well-serviced areas.");
    }

    // General conversation topics
    topics.push("What's your favorite type of music for road trips? I'd love to know what keeps you energized!");
    topics.push("Do you often travel this route, or is this somewhere new for you?");
    topics.push("I'm always curious about people's travel stories. What's the most interesting place you've visited?");
    topics.push("Are you the type who likes to plan every detail of a trip, or do you prefer spontaneous adventures?");
  ...
  static async performSafetyCheck() {
    ...
    if (timeSinceLastUpdate > 120000) { // 2 minutes
      ...
      if (this.locationStationaryTime >= 120) { // 2 minutes stationary
        await this.speak("Hey, I noticed you've been in the same spot for a while. Are you safe? Is everything okay?");
  ...
  static getLocationInfo(locationName) {
    ...
    const locationTips = {
      'downtown': "Downtown areas usually have great food scenes and interesting architecture!",
      'residential': "Residential neighborhoods often have the best local gems and quiet cafes.",
      'business district': "Business districts can have surprisingly good lunch spots and coffee shops.",
      'historic': "Historic areas are full of stories and usually have unique local businesses.",
      'waterfront': "Waterfront areas often have beautiful views and fresh seafood options!",
      'shopping': "Shopping districts are perfect for finding both food courts and upscale dining.",
      'university': "University areas typically have budget-friendly food and vibrant atmosphere.",
    };
  ...
  static async stopNavigation() {
    try {
      ...
      setTimeout(async () => {
        await this.speak("Congratulations! You've arrived at your destination safely. I hope you enjoyed our journey together!");
  ...
  static async speakNavigationUpdate() {
    try {
      ...
      if (!this.isListening || !this.context.isNavigating) {
        console.log("🔇 Navigation stopped or not listening, skipping speak update");
        return;
      }
      ...
      const shouldGiveNavigationInstruction = this.shouldProvideCheckpointInstruction();
      
      if (shouldGiveNavigationInstruction) {
        // Priority 1: Checkpoint-based navigation instructions
        const checkpointInstruction = await this.getCheckpointNavigationInstruction();
        ...
        if (distance <= 50) {
          ...
          if (!this.hasGivenNavigationInstructionForLocation(nextTurn.instruction)) {
            if (this.context.isNavigating) {
              await this.speak(`${nextTurn.instruction} right now!`, { priority: 'high' });
            }
            return;
          }
        }
      ...
      try {
        ...
        if (cityCommentary && typeof cityCommentary === 'string' && cityCommentary.trim().length > 0 && this.context.isNavigating) {
  ...
  static async getCheckpointNavigationInstruction() {
    ...
    try {
      const instruction = await IntelligentAIService.generateCheckpointInstruction({
        checkpoint,
  ...
  static getFallbackCityCommentary(context) {
    const { locationName, nearbyPlaces, speakCount, timeOfDay, routeProgress } = context;
  ...
  static getCurrentCheckpointProgress() {
    ...
    return {
      ...
      progress: this.routeCheckpoints.length > 0 ? (this.checkpointCompleted.size / this.routeCheckpoints.length) * 100 : 0
    };
  }
  ...
  static hasGivenNavigationInstructionForLocation(instruction) {
    ...
    const locationKey = `${Math.round(currentLocation.latitude * 10000)},${Math.round(currentLocation.longitude * 10000)}`;
    ...
    for (const [key, timestamp] of this.navigationInstructionLocations.entries()) {
  ...
  static initializeRouteCheckpoints(selectedRoute) {
    try {
      ...
      if (selectedRoute.overview_polyline && selectedRoute.overview_polyline.points) {
        this.routeCoordinates = this.decodePolyline(selectedRoute.overview_polyline.points);
      ...
      this.routeCheckpoints = this.generateRouteCheckpoints(this.routeCoordinates);
      this.currentCheckpointIndex = 0;
      this.checkpointCompleted.clear();
      
      console.log(`✅ Generated ${this.routeCheckpoints.length} route checkpoints`);
  ...
  static generateRouteCheckpoints(routeCoordinates) {
    ...
    const checkpointInterval = Math.max(1, Math.floor(routeCoordinates.length / 8));
    ...
    for (let i = checkpointInterval; i < routeCoordinates.length - checkpointInterval; i += checkpointInterval) {
      const coord = routeCoordinates[i];
      const distanceFromStart = this.calculateDistanceAlongRoute(routeCoordinates, 0, i);
      const progress = totalDistance > 0 ? (distanceFromStart / totalDistance) * 100 : 0;
      ...
      checkpoints.push({
        ...
        distanceFromStart,
        instruction: this.generateCheckpointInstruction(i, routeCoordinates, progress),
        type: "intermediate"
      });
    }
  ...
  static calculateDistanceAlongRoute(routeCoordinates, startIndex, endIndex) {
    let distance = 0;
    for (let i = startIndex + 1; i <= endIndex && i < routeCoordinates.length; i++) {
  ...
  static checkCheckpointProgress(currentLocation) {
    if (!this.routeCheckpoints.length || this.currentCheckpointIndex >= this.routeCheckpoints.length) {
      return;
    }
  ...
  static async reachCheckpoint(checkpointIndex) {
    ...
    if (this.isListening && checkpoint.instruction) {
      ...
      if (checkpoint.type === 'intermediate') {
        setTimeout(async () => {
          if (this.isListening && this.context.isNavigating) {
            await this.speak(`You're ${checkpoint.progress}% of the way to your destination. Great progress!`);
          }
        }, 2000);
      }
    }
  ...
  static getNavigationUpdateMessages() {
    ...
    if (routeProgress <= 10) {
      messages.push("We're just getting started on our journey. The route looks great ahead!");
      if (locationName) {
        messages.push(`We're beginning our trip from ${locationName}. Let's make this a pleasant journey!`);
      }
    } else if (routeProgress <= 25) {
      messages.push("We're making good initial progress. About a quarter of the way there!");
      messages.push("The first part of our journey is going smoothly. Keep it up!");
    } else if (routeProgress <= 50) {
      messages.push("Excellent! We're approaching the halfway point of our journey.");
      messages.push("You're doing fantastic! We're making steady progress toward our destination.");
    } else if (routeProgress <= 75) {
      messages.push("We're more than halfway there! The end is getting closer.");
      messages.push("Three quarters of the journey complete. You're almost there!");
    } else if (routeProgress <= 90) {
      messages.push("We're in the final stretch! Your destination is coming up soon.");
      messages.push("Almost there! Just a little bit more and we'll have arrived safely.");
    } else {
      messages.push("We're very close to your destination now. Get ready to arrive!");
      messages.push("Final approach to your destination. You've done an amazing job!");
    }
  ...
  static getCityHistoryCommentary(locationName) {
    ...
    if (locationLower.includes('downtown') || locationLower.includes('center')) {
    ...
    } else if (locationLower.includes('old') || locationLower.includes('heritage')) {
    ...
    } else if (locationLower.includes('university') || locationLower.includes('college')) {
  ...
  static getFoodAndCultureCommentary(nearbyPlaces, locationName) {
    const restaurants = nearbyPlaces?.filter(place => 
      place.type?.includes('restaurant') || 
      place.type?.includes('food') ||
      place.category?.includes('dining')
    ) || [];
    
    const cafes = nearbyPlaces?.filter(place => 
      place.type?.includes('cafe') || 
      place.name?.toLowerCase().includes('coffee') ||
      place.name?.toLowerCase().includes('cafe')
    ) || [];
  ...
  static getArchitectureAndLandmarksCommentary(locationName) {
    ...
    if (locationLower.includes('business') || locationLower.includes('commercial')) {
    ...
    } else if (locationLower.includes('residential') || locationLower.includes('neighborhood')) {
    ...
    } else if (locationLower.includes('waterfront') || locationLower.includes('harbor')) {
  ...
  static getLocationBasedMessages() {
    ...
    if (hour >= 6 && hour < 12) {
      messages.push("What a beautiful morning for a drive! The traffic seems light.");
      messages.push("Morning navigation is always pleasant. Hope you're enjoying the journey!");
    } else if (hour >= 12 && hour < 17) {
      messages.push("Perfect afternoon for traveling. The weather looks great for driving.");
      messages.push("Afternoon drives can be so relaxing. How are you feeling about the trip?");
    } else if (hour >= 17 && hour < 22) {
      messages.push("Evening travel has its own charm. The roads are looking good.");
      messages.push("Beautiful evening for navigation. The lighting is perfect for driving.");
    } else {
      messages.push("Night driving requires extra attention. I'm keeping extra watch for you.");
      messages.push("It's quiet out here at night. I'm monitoring everything carefully.");
    }

    // Safety observations
    if (nearbySpots?.some(spot => spot.type === 'police')) {
      messages.push("I notice we're passing through a well-monitored area with police presence nearby.");
    }

    if (nearbySpots?.some(spot => spot.type === 'hospital')) {
      messages.push("This area has good medical facilities nearby. Always reassuring to know!");
    }
    ...
    if (nearbyPlaces?.length > 0) {
      ...
      const cafes = nearbyPlaces.filter(place => 
        place.type?.includes('cafe') || place.name?.toLowerCase().includes('coffee')
      );

      if (restaurants.length > 0) {
        messages.push(`We're passing by ${restaurants[0].name}. Looks like a nice dining spot in this area!`);
      }
      
      if (cafes.length > 0) {
        messages.push(`I can see ${cafes[0].name} around here. Great coffee spots make any area special!`);
      }
    }
    ...
    if (locationName) {
      ...
      if (locationLower.includes('downtown')) {
        messages.push(`We're navigating through ${locationName}. Downtown areas always have so much character!`);
      } else if (locationLower.includes('residential')) {
        messages.push(`Passing through ${locationName}. Residential areas are usually so peaceful for driving.`);
      } else if (locationLower.includes('business')) {
        messages.push(`We're in the ${locationName}. Business districts can be bustling during certain hours.`);
      }
    }
